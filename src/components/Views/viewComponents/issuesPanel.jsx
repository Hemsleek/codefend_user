...
function SourceCode() {
  ...
  createEffect(() => {
    if (!user()) return;
    VoxService.getVulnerabilityResources(user().read_array[0]).then((res) => {
      ...
      <div class="w-full internal-tables internal-tables-scroll">
        <For each={vulnerabilities()}>
          {(vulnerability) => (
            <>
              <div
                class="flex p-3 pl-8 internal-tables-active text-format cursor-pointer"
                onClick={() => {
                  history.push("/issues/" + vulnerability.id);
                }}
              >
                <p class="text-base w-2/12">
                  {new Date(vulnerability.created).toISOString().split("T")[0]}
                </p>
