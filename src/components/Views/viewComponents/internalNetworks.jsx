...
function InternalNetworks() {
  ...
  const { internalNetworksStore, setInternalNetworksStore } =
    createInternalNetwork;

  createEffect(() => {
    if (!user()) return;
    VoxService.getInternalNetworks(user().read_array[0]).then((res) => {
      console.log(res);
  ...
  return (
    <>
      ...
      <div class="w-full internal-tables internal-tables-scroll">
        <For
          each={internalNetworksStore()?.filter(
        ...
        >
          {(network) => (
            <>
              ...
              <For
                each={internalNetworksStore()?.filter(
                  (resource) => resource.is_subdomain === network.id
                )}
              >
                {(subnetwork) => (
                  <div class="flex pl-8 text-format">
                    ...
                    <div class="flex w-3/12">
                      ...
                      <p class="text-base pt-3 pb-3">
                        {subnetwork.internal_ip}
                      </p>
                    </div>
                    <p class="text-base w-2/12 pt-3 pb-3">
                      {subnetwork.external_ip}
                    </p>
                    <p class="text-base w-2/12 pt-3 pb-3">
                      {subnetwork.vendor}
                    </p>
                    <p class="text-base w-3/12 pt-3 pb-3">
                      {subnetwork.network_hostname}
                    </p>
                  </div>
                )}
              </For>
