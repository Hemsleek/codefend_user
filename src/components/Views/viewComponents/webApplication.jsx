...
function InternalNetworks() {
  ...
  createEffect(() => {
    VoxService.getWebResources(user().read_array[0])
      .then((res) => {
        console.log(res)
  ...
  return (
    <>
            ...
            <div class="w-full">
              <div class="w-full w-96 internal-tables disable-border">
                <div class="p-3 internal-tables-active flex">
                  <p class="text-small text-left font-bold title-format">Add a new web resource</p>
                </div>
                <AddDomainModal/>
        ...
        <div onClick={() => {setShowModal(!showModal())}} class="fixed left-0 top-0 flex h-full w-full items-center justify-center bg-black bg-opacity-20 z-20 py-10">
          <div onClick={(e) => {e.preventDefault(); e.stopPropagation()}} class="max-h-full max-w-xl overflow-y-auto bg-white">
            <div class="w-full">
              <div class="w-full w-96 internal-tables disable-border">
                <div class="p-3 internal-tables-active flex">
                  <p class="text-small text-left font-bold title-format">Add a new web subresource</p>
                </div>
                <AddSubDomainModal />
                <div class="container flex items-center justify-center  mx-auto p-3 text-format">
                </div>
      ...
      <div class="p-3 pl-8 internal-tables-active flex">
        <p class="text-small text-left font-bold title-format border-r pr-2">Detected domains and subdomains</p>
        <p onClick={() => {setShowModal(!showModal()); setShowModalStr('add_domain')}} class="text-small text-left font-bold title-format border-r px-2 underline cursor-pointer codefend-text-red">Add domain</p>
        <p onClick={() => {setShowModal(!showModal()); setShowModalStr('add_subdomain')}} class="text-small text-left font-bold title-format border-r px-2 underline cursor-pointer codefend-text-red">Add subdomain</p>
      </div>
            ...
            <>
              <div class="flex p-3 pl-8 internal-tables-active text-format red-border">
                ...
                <section class="flex w-4/12 items-center">
                  <div class={"flag flag-" + mainNetwork.country_code.toLowerCase() + " mr-3"}></div>
                  <p class="text-base">{mainNetwork.country}</p>
                </section>
              </div>
              <For each={resourcesStore().filter(resource => resource.is_subdomain === mainNetwork.id)}>
                {
                  (subNetwork) =>
                  <div class="flex pl-8 text-format">
                    ...
                    <div class="flex w-5/12">
                      ...
                      <p class="text-base pt-3 pb-3">{subNetwork.resource_domain}</p>
                    </div>
                    <p class="text-base w-2/12 pt-3 pb-3">{subNetwork.server_ip}</p>
                    <section class="flex w-4/12 items-center">
                      <div class={"flag flag-" + subNetwork.country_code.toLowerCase() + " mr-3"}></div>
