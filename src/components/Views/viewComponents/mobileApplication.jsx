...
function MobileApplication() {
  ...
  createEffect(() => {
    VoxService.getMobileResources(user().read_array[0])
    .then((res) => {
  ...
  return (
    <>
            ...
            <div class="w-full">
              <div class="w-full w-96 internal-tables disable-border">
                <div class="p-3 internal-tables-active flex">
                  <p class="text-small text-left font-bold title-format">Add a mobile application</p>
                </div>
                <AddMobileModal/>
      ...
      <div class="p-3 pl-8 internal-tables-active flex">
        <p class="text-small text-left font-bold title-format border-r pr-2">Mobile application</p>
        <p onClick={() => {setShowModal(!showModal()); setShowModalStr('add_mobile_app')}} class="text-small text-left font-bold title-format border-r px-2 underline cursor-pointer codefend-text-red">Add application</p>
      </div>
              ...
              <For each={resource}>
                {
                  (subresource) =>
                    <div class="flex pl-8 text-format">
                      ...
                      <p class="text-base w-2/12 pt-3 pb-3">{subresource.app_reviews}</p>
                      <p class="text-base w-1/12 pt-3 pb-3">{subresource.app_rank}</p>
                      <p class="text-base w-2/12 pt-3 pb-3">{subresource.app_version}</p>
                    </div>
                }
              </For>
            </>
